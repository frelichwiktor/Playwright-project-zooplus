...
// JSDoc Example v1
/**
 * @property {Object} page - The page object from Playwright.
 * @property {Object} popup_accept_button - Locator for the accept button in the popup.
 * @property {Object} logo_image - Locator for the logo image.
 * @property {Object} form_login - Locator for the login form.
 */
...
exports.loginPage = class loginPage {
  constructor(page) {
    ...
    this.passwordVisibilityEnable_button = page.locator(
      '#password-visibility-enable',
    );
    this.passwordVisibilityDisable_button = page.locator(
      '#password-visibility-disable',
    );
    this.login_button = page.locator('#login-btn');
    this.forgotPassword_button = page.locator('#forgot-password-link');
    this.invalidLoginData = page.locator(
  ...
  async goToPage() {
    await this.page.goto(
      'https://login.zooplus.pl/auth/realms/zooplus/protocol/openid-connect/auth?response_type=code&client_id=shop-myzooplus-prod-zooplus&redirect_uri=https://www.zooplus.pl/web/sso-myzooplus/login&state=18f790f8-704a-41b1-8e2c-d3dafd27b4bf&login=true&ui_locales=pl-PL&scope=openid',
    );
  }
  ...
  async registerCustomer(firstname, lastname, email, password) {
    ...
    await this.page
      .locator('#shop-header')
      .getByRole('link', { name: 'Mój zooplus' })
      .click();
    await expect(
      this.page.locator('[data-zta="welcome-section-title"]'),
    ).toHaveText(`Cześć, ${firstname}!`);
  }
